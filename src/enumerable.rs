...
pub fn query_owner_allowances(
    ...
    let allowances = ALLOWANCES.prefix(&owner_addr)
...
pub fn query_spender_allowances(
    ...
    let allowances = ALLOWANCES_SPENDER.prefix(&spender_addr)
        .range(deps.storage, start, None, Order::Ascending)
        .take(limit)
        .map(|item| {
...
pub fn query_all_accounts(
    ...
    let accounts = BALANCES.keys(deps.storage, start, None, Order::Ascending)
        .take(limit)
...
mod tests {
    ...
    fn query_all_spender_allowances_works() {
        ...
        // no allowance to start
        let allowances = query_spender_allowances(
            deps.as_ref(),
            spender.clone(),
            None,
            None
        ...
        let allowances: AllSpenderAllowancesResponse = from_binary(
    ...
    fn query_all_accounts_works() {
        ...
        execute(deps.as_mut(), env, info, ExecuteMsg::Transfer {
            recipient: acct4,
